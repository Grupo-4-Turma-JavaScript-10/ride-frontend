  ...
  return (
    <header className="flex bg-gradient-custom font-roboto w-full p-4 items-center">
      ...
      <nav className="font-bold sm:ml-auto flex items-center gap-5 text-black/80">
        ...
        {isOpen && (
          <div
            className="w-full h-full fixed bg-black/10 inset-0"
        ...
        >
          <div className="flex mb-20 items-center">
            <img src={Logo} alt="Velo" className="w-10 scale-130" />
            <h1 className="flex text-2xl">VELO</h1>
          </div>
          ...
          <Link
            to="/cadastro"
            className="bg-custom-yellow rounded-xl p-2 px-6 text-2xl"
            onClick={() => setIsOpen(false)}
          >
            Cadastre-se
          </Link>
        ...
        {navigationItems.map((item) => (
          <Link key={item.path} to={item.path} className="sm:flex hidden">
            {item.label}
          </Link>
        ))}
      </nav>

      <div className="ml-10 flex gap-2 items-center font-bold sm:ml-10">
        <Link to="/login">Login</Link>

        <Link
          to="/cadastro"
          className="bg-custom-yellow rounded-xl p-1 px-3"
        >
          Cadastre-se
        </Link>
      </div>
